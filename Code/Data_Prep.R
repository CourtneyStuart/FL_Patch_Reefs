...
# (patch area, perimeter, perimeter:area, nearest neighbor distance)


#### SUBADULT GRAY SNAPPER (Lutjanus griseus) ####
# reading in the excel data sheet exported from ArcGIS
...
lg = lg %>%
  ...
  # let's make sure that our two presence/absence columns are stored as factors
  mutate(Presence = as.factor(Presence),
         Presence2 = as.factor(Presence2)) %>%
  # add new column (col 58) storing the area sampled by each reef fish survey,
  # where area = pie*radius^2 --> pie*7.5m^2 --> ~176.71 m^2
  mutate(Survey_Area = rep(as.numeric(176.71), nrow(.))) %>%
  # add new column (col 59) storing the density of sub-adult gray snapper,
  # where density = abundance/area of sampling = fish per m^2
  mutate(Density = Abundance/Survey_Area) %>%
  # now keep only the columns we need (selecting by column # rather than name 
  # because it's easier to write), the order here will determine column order
...
lg_vars = lg %>%
  ...
  select(12:34, -Curvature, -Plan_Curve, -Rugosity, -BPI_Fine,
         -Mean_Sum_DO, -Mean_Win_DO)
